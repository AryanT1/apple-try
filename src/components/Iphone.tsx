...
function Model({ item, ...props }: ModelProps) {
  console.log( item)
  const { nodes, materials } = useGLTF('/models/scene.glb') as unknown as GLTFResult
  const texture =  useTexture(item.img) as THREE.Texture
  useEffect(() => {
    if (!materials || !item?.color?.[0]) return
    ...
    Object.entries(materials).forEach(([name, material]) => {
      if (
        ...
        if ('color' in material) {
          (material as THREE.MeshStandardMaterial).color.set(item.color[0])
